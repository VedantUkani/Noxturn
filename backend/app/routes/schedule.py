...
def _proactive_replan(user_id, blocks: List[ScheduleBlock]):
    """Run risk engine + rule planner on the given blocks and persist the result."""
    risk_result = _risk_engine.compute(blocks)
    new_plan = _rule_planner.generate(risk_result, plan_hours=24)
    set_active_plan(str(user_id), new_plan)
    save_plan(user_id, new_plan)
    return new_plan


@router.post("/import", response_model=ScheduleImportResponse)
def import_schedule(request: ScheduleImportRequest, token_user_id: str = Depends(require_user)) -> ScheduleImportResponse:
    ...
    if request.blocks:
        ...
        save_schedule_blocks(request.user_id, normalized, merge_mode=request.merge_mode)

        updated_plan = None
        if report.replan_recommended:
            updated_plan = _proactive_replan(request.user_id, normalized)
    ...
    if request.raw_text:
        ...
        save_schedule_blocks(request.user_id, blocks, merge_mode=request.merge_mode)

        updated_plan = None
        if report.replan_recommended:
            updated_plan = _proactive_replan(request.user_id, blocks)
