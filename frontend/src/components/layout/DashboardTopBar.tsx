...
export function DashboardTopBar() {
  ...
  return (
    <header
      className={cn(
        "flex h-14 shrink-0 items-center gap-4 border-b border-white/[0.06] bg-[#080c14]/92 px-4 backdrop-blur-md md:px-6",
        isToday ? "justify-end" : "justify-between",
      )}
    >
      {!isToday ? (
        <h1 className="min-w-0 truncate text-sm font-semibold text-slate-200 md:text-[0.9375rem]">
          {title}
        </h1>
      ) : null}
